...
/* ── Dual components (DEWE) ────────────────────────────────── */
const DualActivityBlock = ({ activity, assetUrl }) => (
...
const DualGallery = ({ activities, assetUrl }) => {
      ...
      <div className="sheet-body">
        ...
        {assetUrl && (hour.image || hour.images) && (
          <div className="pedagogical-block screenshot-preview">
            <h4 className="block-title">
              {BLOCK_META.screenshot.icon} {BLOCK_META.screenshot.label}
            </h4>
                ...
                {hour.images
                  ? hour.images.map((img, idx) => (
                      <div key={idx} className="screenshot-container">
                        <LazyImg src={assetUrl(img)} alt={`Referencia ${idx + 1}`} className="screenshot-img" />
                      </div>
                    ))
                  : (
                      <div className="screenshot-container">
                        <LazyImg src={assetUrl(hour.image)} alt="Referencia de diseño" className="screenshot-img" />
                      </div>
                    )
                }
              </div>
...
/* ── Week View ─────────────────────────────────────────────── */
const WeekView = ({ weekId, isClassMode, isTeacherMode, isDualMode, isPreviewWeek, curriculumData, assetUrl }) => {
  ...
  const weekMeta   = getWeekMeta(weekId, curriculumData);
  ...
  return (
    <div className={`week-view notebook-view ${isClassMode ? 'class-mode' : ''}`}>
      ...
      <div className="notebook-container" key={dayAnimKey}>
        {isDualMode ? (
          <DualGallery
            activities={weekData.days.find(d => d.id === 'dual')?.activities}
            assetUrl={assetUrl}
          />
        ) : isSpecialDay ? (
              ...
              {currentDay.id === 'dual' ? (
                <div className="dual-repository-view">
                  {currentDay.activities?.map((activity, idx) => (
                    <DualActivityBlock key={idx} activity={activity} assetUrl={assetUrl} />
                  ))}
                </div>
              ) : (
