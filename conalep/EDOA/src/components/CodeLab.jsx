...
const CodeLab = () => {
  ...
  const [htmlCode, setHtmlCode] = useState(() => localStorage.getItem('edoa-codelab-html') || BOILERPLATE.html);
  const [jsCode, setJsCode]     = useState(() => localStorage.getItem('edoa-codelab-js')   || BOILERPLATE.js);
  const [cssFiles, setCssFiles] = useState(() => {
    const saved = localStorage.getItem('edoa-codelab-css');
  ...
  useEffect(() => { localStorage.setItem('edoa-codelab-html', htmlCode); }, [htmlCode]);
  useEffect(() => { localStorage.setItem('edoa-codelab-js',   jsCode);   }, [jsCode]);
  useEffect(() => { localStorage.setItem('edoa-codelab-css',  JSON.stringify(cssFiles)); }, [cssFiles]);
